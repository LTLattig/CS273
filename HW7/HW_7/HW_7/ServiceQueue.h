	...
	void update(int clock)
	{
		...
		if (the_queue.empty()) {
			...
			if (!landing_queue->the_queue.empty()) {
				...
				landing_queue->total_wait += (clock - plane->arrival_time);
				...
				*/
				int randTime = -1;
				while (randTime < min_service_time)
				{
					randTime = my_random.next_int(max_service_time);
				}
				plane->service_time = randTime;
